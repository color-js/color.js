...
let current = $$("#toc > ul > li > a").find((a) => {
	return (
		a.pathname.replace(/\.html|$/i, "") ===
		location.pathname.replace(/\.html|$/i, "")
	);
});
...
if (location.pathname.indexOf("/spaces") > -1) {
	...
	let docsSpaces = Object.fromEntries(
		Mavo.all.colorSpaceData.root.data.space.map((space) => [
			space.id,
			space,
		]),
	);
	...
	let spaces = Object.entries(Color.Space.registry).map(([id, space]) => {
		...
		return Object.assign(
			...
			},
			{
				...
				whitePoint: Object.entries(Color.WHITES).find(
					([name, white]) => white === space.white,
				)?.[0],
	...
	Mavo.all.colorSpaces.dataLoaded
		.then(() => {
