...
let app = createApp({
	data() {
		...
		let spaceId =
			location.pathname.match(/\/picker\/([\w-]*)/)?.[1] ||
			new URL(location).searchParams.get("space");

		if (spaceId && spaceId !== ret.spaceId) {
			ret.coords = new Color(ret.spaceId, ret.coords, ret.alpha / 100).to(
				spaceId,
				{ inGamut: true },
			).coords;
			ret.spaceId = spaceId;
	...
	computed: {
		...
		serialized_color_srgb_oog() {
			return this.color_srgb.toString({
	...
	watch: {
		spaceId(newSpaceId, oldSpaceId) {
			if (newSpaceId != oldSpaceId) {
				...
				let coords = Color.Space.get(oldSpaceId).to(
					newSpace,
					this.coords,
				);
				this.coords = coords;
		...
		color(newColor) {
			requestIdleCallback(() => {
				...
				localStorage.picker_color = JSON.stringify({
					spaceId,
					coords,
					alpha,
				});
