...
for (let method in Color.prototype) {
	if (method.startsWith("deltaE")) {
		...
		deltaE_method.insertAdjacentHTML(
			"beforeend",
...
function render() {
	...
	for (let keyword in KEYWORDS) {
		...
		let deltaE = keywordColor.deltaE(color, {
			method: deltaE_method.value,
		});
	...
	for (let result of results) {
		...
		closest_colors.insertAdjacentHTML(
			"beforeend",
			`
		<article class="keyword-color ${
			keywordColor.oklch.l > L_CUTOFF ? "light-color" : "dark-color"
