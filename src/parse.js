	...
	if (env.parsed) {
		...
		if (name === "color") {
			...
			if (id in ColorSpace.registry) {
				...
				let cssId =
					ColorSpace.registry[id].formats?.functions?.color?.id;
			...
			for (let space of ColorSpace.all) {
				...
				if (format && format.type === "function") {
					...
					if (format.coordGrammar) {
						types = Object.entries(space.coords).map(
							([id, coordMeta], i) => {
								...
								let type;
								if (arg.none) {
									type = coordGrammar.find((c) =>
										noneTypes.has(c),
									);
								} else {
									type = coordGrammar.find(
										(c) => c == providedType,
									);
								}
								...
								if (!type) {
									...
									throw new TypeError(
										`${
											providedType ?? arg.raw
										} not allowed for ${coordName} in ${name}()`,
									);
								}
								...
								let toRange =
									coordMeta.range || coordMeta.refRange;

								if (fromRange && toRange) {
									coords[i] = util.mapRange(
										fromRange,
										toRange,
										coords[i],
									);
